...
function NodeGraph(){
    ...
    if (menu.css("display") == "block"){
      if (e.target.tagName != "LI"){
    	...
    	if(nodes[i]){
    		var n = nodes[i];
    		if(n == currentNode){
    			n.content.css("box-shadow", "10px 10px 5px #888888");
    			n.windowTxt.css("box-shadow", "10px 10px 5px #888888");
    			// n.content.css("border" , "2px solid black");
    			// n.windowTxt.css("border", "2px solid black");
    		}
    		else{
    			n.content.css("box-shadow", "none");
    			n.windowTxt.css("box-shadow", "none");
    			// n.content.css("border" , "1px solid grey");
    			// n.windowTxt.css("border", "1px solid grey");
    		}
    	}
    }
  });
  ...
  $(document).mousemove(function(e){
    ...
    for (var i in nodes){
      if (nodes[i]){
        var n = nodes[i];
        // n.css("box-shadow", "10px 10px 5px #888888");
        if (n != currentNode){
           // n.css("box-shadow", "10px 10px 5px #abe145");
  ...
  function Node(xp, yp, w, h, noDelete, forceId){
	...
	n.append("<textarea style='resize: none;' class='windowTxt' spellcheck='false'/>");
    var windowTxt = $(".node .windowTxt").last();
    windowTxt.css({"position" : "fixed",
    			"height" : "400px",
    			"width" : "300px",
    			"top" : 60+random,
    			"left" : 0+random,
   ...
   // n.append("<div class='srcViewBTN'>SRC");
    n.append("<div class='srcViewBar'>Variable Details");
	

    var srcViewBar = $(".node .srcViewBar").last();
    srcViewBar.css({"position" : "fixed",
    			"height" : "20px", 
    			"width" : "306px",
    			"top" : 40+random,
    			"left" : random,
    ...
    n.append("<div class='srcViewHideBTN'>hide");		 
	var srcViewHideBTN = $(".node .srcViewHideBTN").last();
    
    srcViewHideBTN.css({"position" : "fixed" , "z-index" : 1,
				 ...
				 "border-radius": "5px",
				 "box-shadow": "inset 0 35px 35px -18px #AFCACA, inset 3px 0 12px #700",
				 "background-color": "white",
                 "cursor" : "pointer",
                 "z-index" : 200});
	...
	var boolshow = false;
    srcViewHideBTN.click(function(){
    	if(!boolshow){
    		windowTxt.show('drop');
    		srcViewBar.show('drop');
    		srcViewHideBTN.show(800);
    		boolshow = true;
    	}
    	else{
    		windowTxt.hide('drop');
    		srcViewBar.hide('drop');
    		srcViewHideBTN.hide();
    		boolshow = false;
    	}
    });
 ...
 //------------------------------------------------------------            
    n.append("<div class='srcViewBTN'>VAR");		 
	var srcViewBTN = $(".node .srcViewBTN").last();
    
    srcViewBTN.css({"position" : "absolute" , "z-index" : 1,
                 "width" : "30px", "height" : "15px",
                 "left" : nodeWidth -33 , 
                 "top" : 1,
                 "font-size" : "8px",
    ...
    srcViewBTN.hover(function(){
        srcView.css("background-color","gray")},
		function(){
        srcViewBTN.css("background-color","white");
      });
      //------------------------------------------------------------------
    ...
    n.append("<div class='left' hidden = 'true'>");
    n.append("<div class='top' hidden = 'true'>");
    n.append("<div class='right' hidden = 'true'>");
    n.append("<div class='bottom' hidden = 'true'>");
   ...
   this.remove = function(){
    ...
    resizer.mousedown(function(e){
      ...
      function(){
        ...
        srcViewBTN.css("left", n.width()-33);
    ...
    srcViewBar.mousedown(function(e){
      currentNode = curr;
      n.css("z-index", zindex++);
      e.preventDefault();
      startDrag(windowTxt, {left : 0, top: 60, right : win.width() - 10, bottom : win.height() - windowTxt.height()-srcViewBar.height() +10},
      updateConnections);
      startDrag(srcViewBar, {left : 0, top: 40, right : win.width()- 10, bottom : win.height() - windowTxt.height()-srcViewBar.height() - 10},
      updateConnections);
      startDrag(srcViewHideBTN, {left : windowTxt.width()-27, top: 43, right : win.width()- 7 + windowTxt.width(), bottom : win.height() - windowTxt.height()-srcViewBar.height() - 7},
      updateConnections);
    });
    
    srcViewBTN.mousedown(function(e){
    	currentNode = curr;
      n.css("z-index", zindex++);
    });
    
    windowTxt.mousedown(function(e){
    	currentNode = curr;
      	n.css("z-index", zindex++);
    });
 ...
 function clear(){
  ...
  this.fromJSON = function(data){
    ...
    for (var i in data.nodes){
	  ...
	  var addreturens2 = n.windowTxt.replace(/\\n/g,'\n');
      temp.txt.val(addreturns);
	  temp.windowTxt.val(addreturens2);
    }
  ...
  this.toJSON = function(){
    ...
    for (var i in nodes){
	  ...
	  json += '"windowTxt" : "' + addSlashes(n.windowTxt.val()) + '", ';
